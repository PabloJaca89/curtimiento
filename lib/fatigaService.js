...
// ════════════════════════════════════════════════════════════════════════════
// DUREZA EXPERIMENTADA (medidor semanal miércoles-miércoles)
// 10 = semana durísima (reducir carga) · 1 = semana facilísima (puede subir)
// ════════════════════════════════════════════════════════════════════════════

// Perdón por energía: cuánto se resta a una diferencia percibido-previsto positiva
function perdonPorEnergia(energia) {
  if (energia == null) return 0
  if (energia >= 7) return 0
  if (energia >= 5) return 1
  if (energia >= 3) return 2
  return 3 // energía 1 o 2
}
...
// Devuelve el miércoles que abre la ventana de una fecha dada.
// Cada ventana va de miércoles (incluido) al miércoles siguiente (excluido).
function miercolesDeApertura(fecha) {
  const d = new Date(fecha + 'T12:00:00')
  const dow = d.getDay() // 0=Dom, 3=Mié
  const diff = (dow - 3 + 7) % 7 // días desde el último miércoles
  d.setDate(d.getDate() - diff)
  return aFechaStr(d)
}

// Calcula la dureza (1-10) de una ventana mié-mié dada su fecha de inicio (miércoles).
// Solo cuentan sesiones con planned_load Y perceived_rpe (las creadas por el usuario quedan fuera).
function calcularDurezaVentana(sesiones, semanaInicio) {
  const inicio = new Date(semanaInicio + 'T12:00:00')
  const fin = new Date(inicio)
  fin.setDate(fin.getDate() + 7) // 7 días: mié a mié (fin excluido)

  const dentro = sesiones.filter(s => {
    const f = new Date(s.date + 'T12:00:00')
    return f >= inicio && f < fin &&
      s.planned_load != null && s.perceived_rpe != null
  })

  if (dentro.length === 0) return null

  // Diferencia ajustada por sesión: (percibido - previsto) menos el perdón por energía,
  // pero el perdón solo aplica si la sesión se percibió MÁS dura (diferencia positiva) y nunca cruza 0.
  const diferencias = dentro.map(s => {
    const bruta = s.perceived_rpe - s.planned_load
    if (bruta <= 0) return bruta
    const ajustada = bruta - perdonPorEnergia(s.energy_level)
    return ajustada < 0 ? 0 : ajustada
  })

  const mediaDif = diferencias.reduce((a, b) => a + b, 0) / diferencias.length

  // Convertir a escala 1-10: diferencia 0 -> 5.5 (centro). +duro sube, +fácil baja.
  // Cada punto de diferencia mueve ~1.5 puntos de dureza.
  let dureza = 5.5 + mediaDif * 1.5
  dureza = Math.max(1, Math.min(10, dureza))

  return {
    nota: Math.round(dureza * 100) / 100,
    numSesiones: dentro.length,
  }
}

// Al abrir la app: detecta ventanas mié-mié ya cerradas y sin calcular, y las guarda.
export async function actualizarDurezaSemanal(userId, hoy) {
  const hoyStr = hoy || new Date().toISOString().split('T')[0]

  // Miércoles que abre la semana EN CURSO (aún no cerrada)
  const miercolesActual = miercolesDeApertura(hoyStr)

  // Cargar sesiones de los últimos ~60 días (cubre de sobra 4+ ventanas)
  const inicio = new Date(hoyStr + 'T12:00:00')
  inicio.setDate(inicio.getDate() - 60)
  const inicioStr = aFechaStr(inicio)

  const { data: sesiones } = await supabase
    .from('sessions')
    .select('date, planned_load, perceived_rpe, energy_level')
    .eq('user_id', userId)
    .gte('date', inicioStr)
    .lte('date', hoyStr)
  ...
  // Ventanas ya cerradas: las 4 anteriores a la actual
  const ventanas = []
  for (let i = 1; i <= 4; i++) {
    const d = new Date(miercolesActual + 'T12:00:00')
    d.setDate(d.getDate() - 7 * i)
    ventanas.push(aFechaStr(d))
  }

  // Cuáles ya están guardadas
  const { data: existentes } = await supabase
    .from('weekly_load_ratings')
    .select('semana_inicio')
    .eq('user_id', userId)
    .in('semana_inicio', ventanas)

  const yaGuardadas = new Set((existentes || []).map(e => e.semana_inicio))

  for (const semanaInicio of ventanas) {
    if (yaGuardadas.has(semanaInicio)) continue
    const res = calcularDurezaVentana(sesiones, semanaInicio)
    if (!res) continue // semana sin sesiones válidas: no se guarda
    await supabase.from('weekly_load_ratings').insert({
      user_id: userId,
      semana_inicio: semanaInicio,
      nota: res.nota,
      num_sesiones: res.numSesiones,
    })
  }
}

// Lee las últimas N durezas guardadas (para el recuadro del calendario).
export async function obtenerDurezaSemanal(userId, limite = 4) {
  const { data } = await supabase
    .from('weekly_load_ratings')
    .select('semana_inicio, nota, num_sesiones')
    .eq('user_id', userId)
